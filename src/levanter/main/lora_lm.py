...
def main(config: LoraLmConfig):
    ...
    converter = HFCheckpointConverter.from_hf(config.initialize_from_hf, trust_remote_code=config.trust_remote_code, infer_tokenizer=config.data.tokenizer != "passthrough")
